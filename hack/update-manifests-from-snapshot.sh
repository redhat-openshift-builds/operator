...
function fetch_digests_from_json_snapshot {
    ...
    # List of relatedImages names to match (from CSV) and their internal names
    declare -A relname_to_internal=(
        [OPENSHIFT_BUILDS_OPERATOR]=operator
        [OPENSHIFT_BUILDS_CONTROLLER]=controller
        [OPENSHIFT_BUILDS_GIT_CLONER]=git-cloner
        [OPENSHIFT_BUILDS_IMAGE_PROCESSING]=image-processing
        [OPENSHIFT_BUILDS_IMAGE_BUNDLER]=image-bundler
        [OPENSHIFT_BUILDS_WAITER]=waiters
        [OPENSHIFT_BUILDS_WEBHOOK]=webhook
        [OPENSHIFT_BUILDS_SHARED_RESOURCE_WEBHOOK]=shared-resource-webhook
        [OPENSHIFT_BUILDS_SHARED_RESOURCE]=shared-resource
    )
    local related_images_names=(
        OPENSHIFT_BUILDS_OPERATOR
        OPENSHIFT_BUILDS_CONTROLLER
        OPENSHIFT_BUILDS_GIT_CLONER
        OPENSHIFT_BUILDS_IMAGE_PROCESSING
        OPENSHIFT_BUILDS_IMAGE_BUNDLER
        OPENSHIFT_BUILDS_WAITER
        OPENSHIFT_BUILDS_WEBHOOK
        OPENSHIFT_BUILDS_SHARED_RESOURCE_WEBHOOK
        OPENSHIFT_BUILDS_SHARED_RESOURCE
    )

    for rel_name in "${related_images_names[@]}"; do
        # Normalize: uppercase -> lowercase, _ -> -
        local normalized_name=$(echo "$rel_name" | tr '[:upper:]' '[:lower:]' | tr '_' '-')
        # Find the component in the JSON with a matching name (case-insensitive, dashes/underscores normalized)
        local full_container_image=$(echo "$snapshot_json" | jq -r --arg norm_name "$normalized_name" '
            [.components[] | select((.name | ascii_downcase | gsub("_"; "-") ) == $norm_name)] | if length > 0 then .[0].containerImage else empty end' 2>/dev/null)
        if [ -n "$full_container_image" ]; then
            local current_digest=$(echo "$full_container_image" | awk -F'@' '{print $2}')
            # Save the image name (without digest)
            local image_name_no_digest=$(echo "$full_container_image" | awk -F'@' '{print $1}')
            local internal_name="${relname_to_internal[$rel_name]}"
            if [ -n "$image_name_no_digest" ]; then
                SNAPSHOT_IMAGE_NAME[$internal_name]="$image_name_no_digest"
            fi
            if [ -n "$current_digest" ]; then
                ...
                eval "${digest_var_name}='${current_digest}'"
                found_any_digest=true
                echo "    -> Found '$rel_name' digest: $current_digest (set $digest_var_name)"
                if [ -n "$image_name_no_digest" ]; then
                    echo "    -> Found '$rel_name' image name from snapshot: $image_name_no_digest (set SNAPSHOT_IMAGE_NAME[$internal_name])"
                fi
            else
                echo "    -> Warning: Could not extract digest for '$rel_name' in the JSON. Skipping."
            fi
        else
            echo "    -> Warning: Could not find component for '$rel_name' (normalized: $normalized_name) in the JSON. Skipping."
        fi
    done
