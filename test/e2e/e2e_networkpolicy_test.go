...
var _ = Describe("NetworkPolicy enforcement test", Ordered, Label("e2e"), Label("networkpolicy"), func() {
	...
	Context("NetworkPolicy Deployment", func() {
		...
		It("should have default-deny-all policy deployed", func(ctx SpecContext) {
			...
			fmt.Println("default-deny-all deployed")
		})

		It("should have allow-k8s-api-egress policy deployed", func(ctx SpecContext) {
			np := &networkingv1.NetworkPolicy{}
			err := kubeClient.Get(ctx, types.NamespacedName{
			...
			fmt.Println("K8s API egress deployed")
		})
		...
		It("should apply allow rules before deny-all policy", func(ctx SpecContext) {
			denyAllNP := &networkingv1.NetworkPolicy{}
			err := kubeClient.Get(ctx, types.NamespacedName{
				Name:      "default-deny-all",
				Namespace: openshiftBuildsNS,
			}, denyAllNP)
			...
			allowK8sNP := &networkingv1.NetworkPolicy{}
			err = kubeClient.Get(ctx, types.NamespacedName{
				Name:      "allow-k8s-api-egress",
				Namespace: openshiftBuildsNS,
			}, allowK8sNP)
			...
			Expect(denyAllNP.CreationTimestamp.Time).To(
				BeTemporally(">=", allowK8sNP.CreationTimestamp.Time),
				"default-deny-all should be applied AFTER allow rules to prevent operator lockout")

			timeDiff := denyAllNP.CreationTimestamp.Time.Sub(allowK8sNP.CreationTimestamp.Time)
			fmt.Printf("Applied in correct order: deny-all applied %s after allow rule\n",
				timeDiff.Round(time.Millisecond))
		})
	})

	Context("Egress Policies", Ordered, func() {
		BeforeAll(func() {
			fmt.Println("\n--Testing Egress Policies--")
		})

		AfterAll(func() {
			fmt.Println("\n--Egress Policies Tested--")
		})

		It("should allow egress to Kubernetes API server", func(ctx SpecContext) {
			svc := &corev1.Service{}
			err := kubeClient.Get(ctx, types.NamespacedName{
				Name:      "kubernetes",
				Namespace: defaultNS,
			...
			k8sAPIIP := svc.Spec.ClusterIP
			fmt.Printf("Testing connection to K8s API service: %s:443\n", k8sAPIIP)
			succeeded := testNetworkConnectivityFromNamespace(openshiftBuildsNS, k8sAPIIP, "443")
			Expect(succeeded).To(BeTrue(), "Should be able to connect to K8s API service on port 443")
			fmt.Println("K8s API egress verified")
		})

		It("should block egress to external IPs on non-allowed ports", func(ctx SpecContext) {
			externalTargets := []struct {
				ip          string
				port        string
				description string
			}{
				{"8.8.8.8", "53", "External DNS (port 53)"},
				{"1.1.1.1", "80", "External HTTP (port 80)"},
			}

			for _, target := range externalTargets {
				fmt.Printf("Testing connection to %s:%s (should be blocked)\n", target.ip, target.port)
				succeeded := testNetworkConnectivityFromNamespace(openshiftBuildsNS, target.ip, target.port)
				Expect(succeeded).To(BeFalse(), "%s should be blocked by default-deny-all", target.description)
